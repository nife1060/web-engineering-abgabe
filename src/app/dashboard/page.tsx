  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Students", value: totalStudents.toLocaleString(), icon: "Users", delta: "+12% this month" },
          { label: "Total Revenue", value: `EUR ${totalRevenue.toLocaleString()}`, icon: "EUR", delta: "+8% this month" },
          { label: "Active Courses", value: totalCourses, icon: "Books", delta: "2 in draft" },
          { label: "Avg. Rating", value: `Star ${avgRating}`, icon: "Rate", delta: "Excellent" },
        ].map((stat) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="flex items-end gap-3 h-40">
            {monthlySales.map((month) => (
              <div key={month.month} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs text-gray-500">EUR {(month.sales / 1000).toFixed(1)}k</span>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          ...
          <div className="space-y-3">
            ...
            {["Upload content", "Manage payouts"].map((label) => (
              <button key={label} className="w-full flex items-center gap-3 p-3 rounded-xl border border-gray-100 hover:border-purple-200 hover:bg-purple-50 transition group">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-gray-400">Prototype action</p>
                </div>
              </button>
            ))}
          </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {courseStats.map((course) => (
                <tr key={course.title} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4 text-right text-sm font-semibold text-green-600">EUR {course.revenue.toLocaleString()}</td>
